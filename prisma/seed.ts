...
import { PrismaClient, Domain, ContextLevel } from '@prisma/client';
...
async function main() {
  ...
  const student = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'demo@inquire.dev',
      name: 'Demo Student',
      role: 'STUDENT',
    },
  });
  console.log(`Created/found user: ${student.name} (${student.email})`);

  const educator = await prisma.user.upsert({
    where: { email: 'educator@inquire.dev' },
    update: {},
    create: {
      email: 'educator@inquire.dev',
      name: 'Demo Educator',
      role: 'EDUCATOR',
    },
  });
  console.log(`Created/found user: ${educator.name} (${educator.email})`);
  ...
  const scenarios = [
    ...
    {
      domain: Domain.MATH,
      contextLevel: ContextLevel.SPARSE,
      title: 'City School Construction Decision',
      scenarioText:
        'A school district needs to decide how many new elementary schools to build over the next fifteen years. The district serves a growing city. Construction costs money, and so does leaving children without adequate facilities.',
      contextText: null,
      difficulty: 0.0,
      tags: [] as string[],
    },
    ...
    // ELA
    {
      ...
      scenarioText:
        "In 1923, an anonymous letter of endorsement appeared in a small literary magazine. The letter praised a then-unknown writer's first collection of short stories. That writer went on to significant fame. The letter's author was never publicly identified during their lifetime.",
      contextText: null,
      difficulty: 0.0,
      tags: [] as string[],
    },
    ...
    },
    {
      ...
      scenarioText:
        'The following letter was written in 1847 by a cotton plantation owner in Mississippi to his business factor in New Orleans. It was discovered in a university archive in 1987 and has been studied by historians ever since.',
      contextText:
    ...
    // SCIENCE
    {
      ...
      scenarioText:
        'A hospital in a mid-sized city has reported an unusual cluster of infections among patients in its cardiac surgery unit. The infections have occurred in three separate months over the past year. Hospital administrators and the medical team are trying to understand what is happening.',
      contextText: null,
      difficulty: 0.0,
      tags: [] as string[],
    },
  ...
  for (const scenario of scenarios) {
    const result = await prisma.scenario.create({ data: scenario });
    console.log(`Created scenario: [${result.domain}/${result.contextLevel}] "${result.title}" (difficulty: ${result.difficulty})`);
    created++;
  }

  console.log(`\nSeed complete. Users: 2, Scenarios: ${created}`);
}
